...
# PRD_REF: docs/PRD.md §4.10, §20, §24
...
class CodexRunner:
    ...
    async def _stream(self, context: AgentContext) -> AsyncIterator[AgentEvent]:
        ...
        try:
            ...
            yield AgentEvent(
                kind="error",
                message=f"Codex CLI executable not found: {exc.filename}",
            )
            yield AgentEvent(kind="status", message="failed")
            return
        except OSError as exc:
            yield AgentEvent(kind="error", message=f"Codex CLI failed to launch: {exc}")
            yield AgentEvent(kind="status", message="failed")
            return
        ...
        async for event in _stream_process_output(
            ...
            timeout_message=f"Codex CLI timed out after {self.timeout_seconds:g} seconds",
        ):
            yield event
